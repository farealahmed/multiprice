...
4. **Secrets** — GitHub Actions repository secrets on `farealahmed/multiprice`; see `6-F`'s brief for the exact names. `JWT_SECRET` and the Mongo credentials live in the droplet's `infra/.env` (gitignored, created from `infra/.env.example` on first deploy, never overwritten by later deploys). `DEPLOY_KNOWN_HOSTS` holds an independently verified `known_hosts` entry for the droplet; the workflow must never establish that trust with `ssh-keyscan`.
...
`6-F1` — one unfiltered workflow, `.github/workflows/ci.yml`, so the required checks always complete for every PR. `backend-ci` and `frontend-ci` are separate jobs, each running `actions/setup-node` off the root `.nvmrc` (Node 22), `npm ci`, typecheck, tests, and its production Docker build. The frontend build receives `BACKEND_ORIGIN=http://backend:3001` as a build argument because Next bakes rewrites during `next build`. Each job uploads its image artifact for one day; `deploy` consumes both artifacts from the same run.

1. Protect `main` by requiring the `backend-ci` and `frontend-ci` jobs from the `ci` workflow. Do not reintroduce path filters: a skipped required workflow blocks unrelated PRs.
2. Pin every GitHub Action by full commit SHA, with a nearby human-readable release comment.

`6-F2` — one serialized `deploy` job in that workflow, gated on `push` to `main` (or an explicit `workflow_dispatch` of `main`) and `needs: [backend-ci, frontend-ci]`:
3. Download both artifacts, SSH with `DEPLOY_SSH_KEY` to `DEPLOY_USER@DEPLOY_HOST`, and rsync `infra/` and both tarballs to the droplet. Use the independently verified `DEPLOY_KNOWN_HOSTS` secret with strict host-key checking; do not use `ssh-keyscan`.
4. Over the same SSH connection: create `infra/.env` from `infra/.env.example` if missing (never overwrite an existing one), load both images, then run `docker compose -f infra/compose.yml up -d --no-build`. A single deployment job prevents an initial boot without both local images and prevents overlapping workflows from rolling a service back.
5. Smoke-test both `https://multiprice.farealahmed.com/api/health` and `https://multiprice.farealahmed.com/` with a ~10 try / 5s retry loop.
6. Secrets referenced as `${{ secrets.NAME }}`: `DEPLOY_SSH_KEY`, `DEPLOY_HOST` (`multiprice.farealahmed.com`), `DEPLOY_USER` (`deploy`), and `DEPLOY_KNOWN_HOSTS` (the verified host entry). **You cannot add these values yourself** — the human sets them in repository settings.
7. Do not automate the browser smoke test from `6-E`'s Done criteria — the curl health check above is enough for CD; the sign-up → `421.50` → finalize walkthrough stays a manual step in `J6`.
